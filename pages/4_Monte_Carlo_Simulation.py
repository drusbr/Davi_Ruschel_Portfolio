...
def monte_carlo_safety_car(laps, drivers, race, sims=1000, seed=42):
    ...
    laps_previous = pd.read_csv("laps_all_seasons.csv")
    laps_previous_season = laps_previous[laps_previous["season"]==2024]
    laps_previous_circuit = laps_previous_season[laps_previous_season["grand_prix"]==race]
    avg_lap = laps_previous_circuit["lap_time_s"].mean()
    threshold = avg_lap * 0.5

    for driver in drivers:
        avg_pace[driver], avg_std[driver] = get_race_stats(race, driver)
        if avg_pace[driver] < threshold:
            avg_pace[driver] = avg_lap*1.3
...
options = st.radio("Choose your Option", ("Monte-Carlo Analysis", "Single Race Analysis"), horizontal=True)

schedule = ff1.get_event_schedule(2025)
races = schedule["EventName"]
races = races[1:15]

if options == "Monte-Carlo Analysis":
    ...
    st.subheader("Single Race (Driver Focus)")
    ...
    drivers_abbv, race_laps = load_session(race)

    with config2:
        driver_selection = st.selectbox("Driver", drivers_abbv)

    # -------------------- ADDED: persist results & control recompute --------------------
    # init state
    if "df_laps" not in st.session_state:
        st.session_state.df_laps = None
    if "df_summary" not in st.session_state:
        st.session_state.df_summary = None

    # button to (re)run the simulation; otherwise keep previous results
    run_now = st.button("Run Simulation")

    if run_now or st.session_state.df_laps is None:
        df_laps, df_summary = monte_carlo_safety_car(int(race_laps), drivers_abbv, race, sims=1, seed=42)

        df_laps = pd.read_csv("monte_carlo_laps.csv")
        df_summary = pd.read_csv("monte_carlo_summary.csv")

        st.session_state.df_laps = df_laps
        st.session_state.df_summary = df_summary
    else:
        df_laps = st.session_state.df_laps
        df_summary = st.session_state.df_summary
    # -----------------------------------------------------------------------------

    # df_racecontrol = pd.read_csv("race_control.csv")
    # df_decision = pd.read_csv("decision_making.csv")

    analysis_choice = st.radio(
        "Choose the Analysis:",
        ["Race Overview", "Driver-Specific", "Head-to-Head"], index=0, horizontal=True)

    if analysis_choice == "Race Overview":

        last_rows = (df_laps.sort_values(["Driver", "Lap"]).groupby("Driver", as_index=False).tail(1))

        race_summary_end = (last_rows[["Driver", "Position", "CumulativeLapTime", "IntervalAhead", "GapToLeader"]].sort_values("Position").reset_index(drop=True))

        race_summary_end["CumulativeLapTime"] = race_summary_end["CumulativeLapTime"].map(lambda x: f"{x:.3f} s")
        race_summary_end['IntervalAhead']  = race_summary_end['IntervalAhead'].map(lambda x: (f"+{x:.3f}s") if pd.notna(x) and x!=0 else "")
        race_summary_end['GapToLeader']    = race_summary_end['GapToLeader'].map(lambda x: (f"+{x:.3f}s") if pd.notna(x) and x!=0 else "Leader")

        race_summary_end_dict = race_summary_end.to_dict(orient="list")

        st.subheader("Final Classification")
        st.dataframe(race_summary_end_dict, use_container_width=True)

        # Race Statistics
        st.subheader("Race Statistics/Summary:")
        st.write(f"Fastest Lap: {df_laps['LapTime'].min():.3f} seconds")
        st.write(f"Safety Car: {df_summary['SC_Happens'].iloc[0]}")

        drivers_choice = st.radio("Choose Drivers to Analyse:", ["All Drivers", "Choose my Own"], index=0, horizontal=True)

        if drivers_choice == "All Drivers":
            df_laps = df_laps
        else: 
            selected_drivers = st.multiselect("Choose Drivers:", options=drivers_abbv, placeholder="Pick One or More Drivers...")
            df_laps = df_laps[df_laps["Driver"].isin(selected_drivers)]
            min_laptime = df_laps["LapTime"].min()
            max_laptime = df_laps["LapTime"].max()
        
        for driver in drivers_abbv:
            df_driver = df_laps[df_laps["Driver"]==driver]
            if df_driver["LapTime"].mean() < 5:
                df_laps = df_laps[df_laps["Driver"] != driver]
                min_laptime = df_laps["LapTime"].min()
                max_laptime = df_laps["LapTime"].max()
        
        for driver in df_laps["Driver"].unique():
            df_driver = df_laps[df_laps["Driver"] == driver]
            # mean(skipna=True) and also handle all-NaN / empty driver slice
            if df_driver["LapTime"].dropna().empty or df_driver["LapTime"].mean(skipna=True) < 5:
                df_laps = df_laps[df_laps["Driver"] != driver]

        if df_laps.empty or df_laps["LapTime"].dropna().empty:
            st.warning("No valid lap time data to pllot after filtering.")
            st.stop()

        min_laptime = df_laps["LapTime"].min()
        max_laptime = df_laps["LapTime"].max()

    #y=alt.Y("LapTime:Q", scale=alt.Scale(domain=[min_laptime-10,max_laptime+10]))

        lap_time_chart = alt.Chart(df_laps).mark_line().encode(
            x="Lap:Q", y=alt.Y("LapTime:Q", scale=alt.Scale(domain=[min_laptime-10,max_laptime+10])), color="Driver:N"
        )
        ...
        race_pos_chart = alt.Chart(df_laps).mark_line().encode(
            x="Lap:Q", y="Position:Q", color="Driver:N"
        )
        ...
        order = (df_laps.groupby("Driver", as_index=False)["LapTime"].median().sort_values("LapTime", ascending=True)["Driver"].tolist())
        
        box_plot_laptimes = alt.Chart(df_laps).mark_boxplot().encode(
            x=alt.X("Driver:N", title="Driver", sort=order), y=alt.Y("LapTime:Q", title="Lap Time [s]", scale=alt.Scale(domain=[min_laptime-2, max_laptime+2])), color="Driver:N")

        st.subheader("Driver-Specific Box Plot")
        st.altair_chart(box_plot_laptimes, use_container_width=True)

        gap_to_leader = alt.Chart(df_laps).mark_line().encode(
            x="Lap:Q", y=alt.Y("GapToLeader:Q"), color="Driver:N")
        
        st.subheader("Gap to Leader")
        st.altair_chart(gap_to_leader, use_container_width=True)
